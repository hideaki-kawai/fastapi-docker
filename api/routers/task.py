...
async def create_task(task_body: task_schema.TaskRequest, db: Session = Depends(get_db)):
    try:
...
async def update_task(task_id: int, task_body: task_schema.TaskRequest, db: Session = Depends(get_db)):
